...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const scriptPath = path.resolve(process.cwd(), 'data/fetch_shares_outstanding_data.py');
  ...
  return new Promise<Response>((resolve) => {
    ...
    const run = (pythonCmd: string) => {
      ...
      py.stderr.on('data', (err) => { console.error('[shares_outstanding_data] stderr:', err.toString()); });
